...
def get_actual_icon(title):
    # Search for title in a more flexible way
    # Match title inside h4 tags or alt tags
    for piece in card_pieces:
        # Check if title exists as plain text inside a tag
        if re.search(r'>(.*?)' + re.escape(title) + r'(.*?)<', piece, re.IGNORECASE):
...
print(f"Generated {generated_count} individual pages with fixed icon paths.")
