...
# Re-read index.html to find the actual icon paths
with open('index.html', 'r', encoding='utf-8') as f:
    index_content = f.read()

# Pre-parse the index.html into game cards to make icon finding accurate
# Each card looks like: <div class="game-card" ...> ... </div> </div> </div>
# We split by <div class="game-card" and search within each piece
card_pieces = re.split(r'<div class="game-card"', index_content)[1:]

def get_actual_icon(title):
    title_clean = title.lower().strip()
    for piece in card_pieces:
        # Check if this card piece contains the title
...
for entry in all_entries:
    ...
    icon_path = get_actual_icon(entry['title'])
...
print(f"Generated {generated_count} individual pages with correct icons.")
