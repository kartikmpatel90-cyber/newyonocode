...
# Separate the file into game cards
# A card starts with class="game-card" and ends before the next card or list end
# We'll use a more reliable split
card_blocks = re.findall(r'<div class="game-card".*?</div>\s*</div>\s*</div>', index_content, flags=re.DOTALL)

def get_actual_icon_robust(title):
    slug = re.sub(r'[^a-z0-9]', '', title.lower())
    search_term = f'/{slug}/'
    
    for block in card_blocks:
        # Check if this block contains the unique slug link we added
        if search_term in block or title.lower() in block.lower():
            # Found the correct card block!
            # Now extract the first image src
...
<head>
    ...
    <style>
        ...
        .app-icon {{ width: 120px; height: 120px; border-radius: 24px; box-shadow: 0 10px 20px rgba(0,0,0,0.1); object-fit: cover; }}
...
for entry in all_entries:
    ...
    icon_path = get_actual_icon_robust(entry['title'])
...
print(f"Generated {generated_count} individual pages with ROBUST icon detection.")
