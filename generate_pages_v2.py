...
def get_actual_icon(title):
    # Find the image src near the title
    # We look for the game-card that contains the title
    # Then find the img tag src
    pattern = r'<h4 class="game-title">' + re.escape(title) + r'.*?<img src="(.*?)"'
    match = re.search(pattern, index_content, re.DOTALL | re.IGNORECASE)
    if not match:
        # Try finding the title as alt text
        pattern = r'<img src="(.*?)" alt="' + re.escape(title) + r'"'
        match = re.search(pattern, index_content, re.DOTALL | re.IGNORECASE)
...
print(f"Generated {generated_count} individual pages.")
